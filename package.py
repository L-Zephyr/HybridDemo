...
def package(route):
    ...
    for dirpath, dirnames, filenames in zip_files:
        for filename in filenames:
            if filename == "package.py" or unicode(filename, "utf-8") == zip_file_name:  # 防止打包脚本和资源文件在同一个目录
                continue
...
def setup_package(info={}):
    ...
    设置资源包的信息
    '''
    route = info[ROUTE] if info.has_key(ROUTE) else raw_input("请输入资源包的路由URL:")
    entrance = info[ENTRANCE] if info.has_key(ENTRANCE) else raw_input("请指定资源包的入口文件:")
    version = raw_input("请输入版本号(当前版本" + str(info[VERSION]) + "):") if info.has_key(VERSION) else raw_input("请输入版本号:")
...
def main():
    ...
    print "打包完成，资源包摘要为:" + encrypted_hash
